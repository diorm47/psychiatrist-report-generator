      ...
      if (inputs[i].type == 'text'){
          ...
          if (inputs[i].value != ""){
              ...
              if (inputs[i].placeholder == "Namen von Symptomen"){
                  report.textContent = report.textContent + 'Auswirkungen der durchgeführten Behandlung auf spezifische psychopathologische Symptome: ' + inputs[i].value + ': ';
              }else if(inputs[i].name == 'termination-other-text') {
                  report.textContent =  report.textContent + 'Umstände der Behandlungsbeendigung: ' + inputs[i].value + '\n'; 
              }else{
                  if (legend){
          ...
          if (inputs[i].value != ""){
              ...
              if (inputs[i].checked){
                  const fieldset = inputs[i].closest('fieldset');
                  const legend = fieldset.querySelector('legend');
                  if (inputs[i].parentElement.querySelector('label').innerText == "Auswirkungen der durchgeführten Behandlung auf spezifische psychopathologische Symptome:"){
          ...
          const fieldset = inputs[i].closest('fieldset');
          const legend = fieldset.querySelector('legend');
          if (parent.parentElement.querySelector('label').textContent == 'Probleme im Zusammenhang mit Angst:'){
              if (inputs[i].checked){
                  if(!report.textContent.includes(parent.parentElement.querySelector('label').textContent)){
                      if (legend){
                          if (!report.textContent.includes(legend.textContent)){
                              report.textContent = report.textContent + legend.textContent + '\n';
                          }
                      }
                      report.textContent = report.textContent + '\n' + parent.parentElement.querySelector('label').textContent + ' ' + inputs[i].value;
                  } else{

                      report.textContent = report.textContent + ', ' +  inputs[i].value;
                  }
              }
          }else if(legend.textContent == "Interventionen der kognitiv-behavioralen Therapie (CBT)"){
              if (inputs[i].checked){
                  if(!report.textContent.includes('Interventionen der kognitiv-behavioralen Therapie (CBT)')){
                      report.textContent = report.textContent + '\n' + 'Psychotherapeutische Interventionen';
                      report.textContent = report.textContent + '\n' + 'Interventionen der kognitiv-behavioralen Therapie (CBT): ' + inputs[i].value;
                  } else{

                      report.textContent = report.textContent + ', ' +  inputs[i].value;
                  }
              }
          }else if(parent.parentElement.querySelector('label').innerText == "Probleme im Zusammenhang mit Emotionen, Stimmung und Emotionsregulation:"){
              if (inputs[i].checked){
                  if(!report.textContent.includes('Probleme im Zusammenhang mit Emotionen, Stimmung und Emotionsregulation:')){
                      report.textContent = report.textContent + '\n' + 'Probleme im Zusammenhang mit Emotionen, Stimmung und Emotionsregulation:' + inputs[i].value;
                      if (legend){
                          if (!report.textContent.includes(legend.textContent)){
                              report.textContent = report.textContent + legend.textContent + '\n';
                          }
          ...
          }else if (parent.parentElement.querySelector('label').innerText == "Probleme im Zusammenhang mit Impulskontrolle:"){
              if (inputs[i].checked){
                  if (!report.textContent.includes("Probleme im Zusammenhang mit Impulskontrolle:")){
                      if (legend){
                          if (!report.textContent.includes(legend.textContent)){
                              report.textContent = report.textContent + legend.textContent + '\n';
                          }
                      }
                      report.textContent = report.textContent + '\n' + "Probleme im Zusammenhang mit Impulskontrolle:" + inputs[i].value;
                  }else{
                      report.textContent = report.textContent + ', ' + inputs[i].value;
                  }
              }
          }else if(parent.parentElement.querySelector('label').innerText == "Probleme im Zusammenhang mit Kommunikation und Beziehungen:"){
              if (inputs[i].checked){
                  if (!report.textContent.includes("Probleme im Zusammenhang mit Kommunikation und Beziehungen:")){
                      if (legend){
                          if (!report.textContent.includes(legend.textContent)){
                              report.textContent = report.textContent + legend.textContent + '\n';
                          }
                      }
                      report.textContent = report.textContent + '\n' + "Probleme im Zusammenhang mit Kommunikation und Beziehungen:" + inputs[i].value;
                  }else{
                      report.textContent = report.textContent + ', ' + inputs[i].value;
                  }
              }
          }else if(parent.parentElement.querySelector('label').innerText == "Probleme im Zusammenhang mit den kognitiven Funktionen:" || parent.parentElement.querySelector('label').innerText == "Probleme im Zusammenhang mit eigenem Selbst:" || parent.parentElement.querySelector('label').innerText == 
          "Probleme im Zusammenhang mit der psychischen Anpassung:" ||  parent.parentElement.querySelector('label').innerText == "Probleme im Zusammenhang mit der Erfahrung von Trauma und Missbrauch:" ||  parent.parentElement.querySelector('label').innerText == 
          "Probleme im Zusammenhang mit körperlichen und psychosomatischen Faktoren:" ||  parent.parentElement.querySelector('label').innerText == "Sonstige Probleme:"){
              if (inputs[i].checked){
                  if (!report.textContent.includes(parent.parentElement.querySelector('label').textContent)){
                      if (legend){
                          if (!report.textContent.includes(legend.textContent)){
              ...
              if (!report.textContent.includes(parent.parentElement.parentElement.querySelector('legend').textContent)){
                  ...
                  const legend = fieldset.querySelector('legend');
                  if (!report.textContent.includes(legend.textContent)){
                      report.textContent = report.textContent + '\n' + legend.textContent + ': ' + inputs[i].value;
                  }else{
                      report.textContent = report.textContent + ', '  + inputs[i].value.split('\n');
                  }
